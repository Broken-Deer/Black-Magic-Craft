    ...
    await new Promise(function (resolve, reject) {
        ...
        .then((version_data) => {
            ...
            for (let i = 0; i < lib.length; i++) {
                ...
                if (allow == true) {
                    /* 如果文件夹不存在就先创建 */
                    makeDir((libPath + lib[i]["downloads"]["artifact"]["path"]).replace(/(?=(\/)(?!.*\1)).*?$/g, ""));
                    const downloadStream = got.stream(lib[i]["downloads"]["artifact"]["url"]);
                    const fileWriterStream = f.createWriteStream(libPath + lib[i]["downloads"]["artifact"]["path"]);
                    /* 放置侦测器监听下载 */
                    downloadStream.on("error", () => {});
                    fileWriterStream
                        .on("error", () => {
